...
if (CLEAR_SLASH_COMMAND_API_DATA === 'true') clearApplicationCommandData();
...
for (const file of getFiles(CHAT_INPUT_COMMAND_DIR)) {
  try {
    ...
    cmd.loadAliases();
  } catch (e) { logger.syserr(`Error loading command: ${file}`); console.error(e); }
}
...
['user', 'message'].forEach(type => {
  ...
  for (const file of getFiles(`${CONTEXT_MENU_COMMAND_DIR}/${type}`)) {
    ...
    } catch (e) { logger.syserr(`Error loading ${type} context menu: ${file}`); console.error(e); }
  }
});

// Ładowanie reszty interakcji
const dirs = {
...
Object.entries(dirs).forEach(([name, dir]) => {
  ...
  for (const file of getFiles(dir)) {
    try {
      const cmd = require(file);
      cmd.load(file, client.container[name]);
    } catch (e) { logger.syserr(`Error loading ${name}: ${file}`); console.error(e); }
  }
});

refreshSlashCommandData(client);

// Register other listeners from /listeners folder (except interactionCreate)
...
// === GŁÓWNY HANDLER INTERAKCJI - WYMUSZONY NA KOŃCU ===
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    logger.syserr(`[INTERACTION] Błąd krytyczny przy ${interaction.commandName}`);
    console.error(err);
    if (!interaction.replied && !interaction.deferred) {
      interaction.reply({ content: '❌ Wystąpił krytyczny błąd bota.', ephemeral: true }).catch(() => {});
    }
  }
});
...
app.listen(PORT, () => console.log(`[KEEP-ALIVE] Nasłuchuję na porcie ${PORT} – Render szczęśliwy 🚀`));
